def largest_Product_in_a_Grid(dizi):
  ...
  for i in range(0, len(dizi)):
    for j in range(0, len(dizi)):
      ...
      sonucListe.append(sag)
      solAltCapraz = solAltCarp(dizi, i, j)
      sonucListe.append(solAltCapraz)
      sagAltCapraz = sagAltCaprazCarp(dizi, i, j)
      sonucListe.append(sagAltCapraz)
      sonuc = enBuyukBul(sonucListe)
      
      carpimListesi.append(sonuc)
  enBuyukCarpimSonucu = carpimlarinEnBuyugu(carpimListesi)
...
def carpimlarinEnBuyugu(carpimListesi):
  sonuc = carpimListesi[0]
  for i in range(0, 400):
    if(sonuc < carpimListesi[i]):
      sonuc = carpimListesi[i]
  return sonuc

dizi = [[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
...
sonuc = largest_Product_in_a_Grid(dizi)
print(sonuc)